...
<body style="background-image: url('img/admin_form_bg.jpg'); background-repeat: no-repeat;">
  ...
  $sql = "SELECT * FROM appointments where d_id = '" . $_SESSION['id'] . "' ORDER BY id";
            ...
            ?>
              <tr>
                ...
                <?php elseif ($rows['approve'] == 1) : ?>
                  <td>Approved</td>
                <?php else : ?>
                  <td><button type="button" title="Cancel Appointment" class="btn btn-danger" onclick="cancelRow(<?php echo $rows['id']; ?>)"><i class="bi bi-calendar2-x"></i></button>
                    <button type="button" title="Approve Appointment" class="btn btn-success" onclick="approveRow(<?php echo $rows['id']; ?>)"><i class="bi bi-calendar-check"></i></button>
                  </td>
                <?php endif; ?>
              </tr>
            <?php
